#!/bin/bash
#
# MAE training submission to SDCC GPU pool.
#
# Usage:
#   bash gridutils/submit_mae.sh <path/to/run_config.json>
#
...
# debug flags) is read from the JSON by mae.scripts.train_mae.from_config
# on the worker node.
...
if [ -z "$run_name" ] || [ "$run_name" = "None" ] || [ "$run_name" = "null" ]; then
...
universe                = vanilla
notification            = never
executable              = ${REPODIR}/gridutils/trainjob_mae.sh
