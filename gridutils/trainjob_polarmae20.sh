#!/bin/bash
# trainjob_polarmae20.sh — worker-side launcher for the 20-epoch full-1M
...
cd "$polar_repo"
echo "Launching run_polarmae_fit.py fit (${run_name}) ..."
"$PY" -u "${larmamba_repo}/gridutils/run_polarmae_fit.py" fit \
    --config "$config" \
